	...
	while(m_emuState != EMU_QUIT)
	{
		...
		if ( isRunning && !isPaused)
		{
			...
			if ( tmp >= m_FPSUpdateInterval ) 
			{
				...
				{
					if ( g_tlcs900hDebugger.state() == DEBUGGER_BREAK )
					{
						SendMessage(g_tlcs900hDebugHwnd, WM_COMMAND, IDC_TLCS900HD_PAUSE, 0);
						SetDlgItemText(g_tlcs900hDebugHwnd, IDC_TLCS900h_DEBUGGER_STATUS, "Breakpoint Hit");
					}
					else if ( g_tlcs900hDebugger.state() == DEBUGGER_PAUSE && g_tlcs900hUpdateDebug == true )
					{
						SendMessage(g_tlcs900hDebugHwnd, WM_COMMAND, IDC_TLCS900HD_PAUSE, 0);
				...
				// Assuming 60fps runs at about 13.33 ms per update, give 10 ms back to the cpu
				Sleep(10);
			}
		...
		{
			// (CPU cool-down) nothing happening during pause, give it all back
			Sleep(10);
		}
