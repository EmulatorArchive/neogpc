...
{
	unsigned char i = z80MemReadByte(z80Regs[Z80PC]);
...
inline unsigned short z80Readword()
{
	unsigned char i = z80MemReadByte(z80Regs[Z80PC]);
	z80Regs[Z80PC]+=1;
	unsigned char j = z80MemReadByte(z80Regs[Z80PC]);
	z80Regs[Z80PC]+=1;
	return (j<<8)|i;
}
...
{
	*z80RegsB[(z80Opcode>>3)&7] = z80MemReadByte(z80Regs[Z80HL]);	return 8;
}

int z80LDHLr()		// LD (HL),r
{
	z80MemWriteByte(z80Regs[Z80HL],*z80RegsB[z80Opcode&7]);	return 4+3;
}

int z80LDHLn()		// LD (HL),n
{
	z80MemWriteByte(z80Regs[Z80HL],z80Readbyte());	return 4+3+3;
}

int z80LDArr()		// LD A,(rr)		(BC/DE)
{
	*z80RegsB[Z80A] = z80MemReadByte(z80Regs[(z80Opcode>>4)&1]);	return 4+3;
}

int z80LDAnn()		// LD A,(nn)
{
	*z80RegsB[Z80A] = z80MemReadByte(z80Readword());	return 4+3+3+3;
}

int z80LDrrA()		// LD (rr),A		(BC/DE)
{
	z80MemWriteByte(z80Regs[(z80Opcode>>4)&1],*z80RegsB[Z80A]);	return 4+3;
}

int z80LDnnA()		// LD (nn),A
{
	z80MemWriteByte(z80Readword(),*z80RegsB[Z80A]);	return 4+3+3+3;
}

int z80LDmemHL()	// LD (nn),HL
{
	z80MemWriteWord(z80Readword(),z80Regs[Z80HL]);	return 4+3+3+3+3;
}

int z80LDnnrr()		// LD (nn),rr
{
	z80MemWriteWord(z80Readword(),z80Regs[(z80Opcode>>4)&3]);	return 4+4+3+3+3+3;
}

int z80LDrrnnm()	// LD rr,(nn)
{
	z80Regs[(z80Opcode>>4)&3] = z80MemReadWord(z80Readword());	return 4+4+3+3+3+3;
}

int z80LDHLmem()	// LD HL,(nn)
{
	z80Regs[Z80HL] = z80MemReadWord(z80Readword());	return 4+3+3+3+3;
}
...
{
	z80MemWriteWord(z80Readword(),z80Regs[Z80SP]);	return 20;
}
...
{
	*z80Index = z80MemReadWord(z80Readword());	return 4+3+3+3+3;
}
...
int z80LDXYn()		// LD (Iz+d8),n
{
	...
	z80MemWriteByte(*z80Index+d8,z80Readbyte());	return 4+3+5+3;
}
...
int z80LDXYr()		// LD (Iz+d8),r
{
	...
	z80MemWriteByte(*z80Index+d8,*z80RegsB[z80Opcode&7]);	return 4+3+5+3;
}
...
int z80LDXYH()		// LD (Iz+d8),H
{
	...
	z80MemWriteByte(*z80Index+d8,*(((unsigned char *)&z80Regs[Z80HL])+1));	return 4+3+5+3;
}
...
int z80LDXYL()		// LD (Iz+d8),L
{
	...
	z80MemWriteByte(*z80Index+d8,*((unsigned char *)&z80Regs[Z80HL]));	return 4+3+5+3;
}

int z80LDnnXY()		// LD (nn),Iz
{
	z80MemWriteWord(z80Readword(),*z80Index);	return 4+3+3+3+3;
}
...
int z80LDrXY()		// LD r,(Iz+d8)
{
	...
	*z80RegsB[(z80Opcode>>3)&7] = z80MemReadByte(*z80Index+d8);	return 4+3+5+3;
}
...
int z80LDHXY()		// LD H,(Iz+d8)
{
	...
	*(((unsigned char *)&z80Regs[Z80HL])+1) = z80MemReadByte(*z80Index+d8);	return 4+3+5+3;
}
...
int z80LDLXY()		// LD L,(Iz+d8)
{
	...
	*((unsigned char *)&z80Regs[Z80HL]) = z80MemReadByte(*z80Index+d8);	return 4+3+5+3;
}
...
int z80LDD()		// LDD
{
	z80MemWriteByte(z80Regs[Z80DE],z80MemReadByte(z80Regs[Z80HL]));
...
{
	z80Regs[Z80SP]-= 2;	z80MemWriteWord(z80Regs[Z80SP],z80Regs[(z80Opcode>>4)&3]);	return 5+3+3;
}

int z80PUSHAF()		// PUSH AF
{
	z80Regs[Z80SP]-= 2;	z80MemWriteWord(z80Regs[Z80SP],z80Regs[Z80AF]);	return 5+3+3;
}

int z80PUSHXY()		// PUSH Iz
{
	z80Regs[Z80SP]-=2;	z80MemWriteWord(z80Regs[Z80SP],*z80Index);	return 5+3+3;
}
...
{
	z80Regs[(z80Opcode>>4)&3] = z80MemReadWord(z80Regs[Z80SP]);	z80Regs[Z80SP]+= 2;	return 4+3+3;
}

int z80POPAF()		// POP AF
{
	z80Regs[Z80AF] = z80MemReadWord(z80Regs[Z80SP]);	z80Regs[Z80SP]+= 2;	return 4+3+3;
}

int z80POPXY()		// POP Iz
{
	*z80Index = z80MemReadWord(z80Regs[Z80SP]);	z80Regs[Z80SP]+=2;	return 4+3+3;
}
...
{
	*z80RegsB[Z80A] = z80AddB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80ADDAXY()		// ADD A,(Ix+d)
{
	...
	*z80RegsB[Z80A] = z80AddB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}
...
{
	*z80RegsB[Z80A] = z80AdcB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
{
	*z80RegsB[Z80A] = z80SubB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80SUBAXY()		// SUB A,(Iz+d8)
{
	...
	*z80RegsB[Z80A] = z80SubB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}
...
{
	*z80RegsB[Z80A] = z80SbcB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
{
	*z80RegsB[Z80A] = z80AndB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80ANDAXY()		// AND A,(Iz+d8)
{
	...
	*z80RegsB[Z80A] = z80AndB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}
...
{
	*z80RegsB[Z80A] = z80OrB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80ORAXY()		// OR A,(Iz+d8)
{
	...
	*z80RegsB[Z80A] = z80OrB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}
...
{
	*z80RegsB[Z80A] = z80XorB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80XORAXY()		// XOR A,(Iz+d8)
{
	...
	*z80RegsB[Z80A] = z80XorB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}
...
{
	z80SubB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));	return 4+3;
}
...
int z80CPAXY()		// CP A,(Iz+d8)
{
	...
	z80SubB(*z80RegsB[Z80A],z80MemReadByte(*z80Index+d8));	return 4+3+5+3;
}

int z80CPD()		// CPD
{
	z80SubB(*z80RegsB[Z80A],z80MemReadByte(z80Regs[Z80HL]));
...
int z80INCHL()		// INC (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80IncB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+3;
}
...
int z80INCXYm()		// INC (Iz+d8)
{
	...
	z80MemWriteByte(*z80Index+d8,z80IncB(z80MemReadByte(*z80Index+d8)));	return 4+3+5+4+3;
}
...
int z80DECHL()		// DEC (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80DecB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+3;
}
...
int z80DECXYm()		// DEC (Iz+d8)
{
	...
	z80MemWriteByte(*z80Index+d8,z80DecB(z80MemReadByte(*z80Index+d8)));	return 4+3+5+4+3;
}
...
int z80RLCHL()		// RLC (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80RlcB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80RLCXY()		// RLC (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80RlcB(*z80Index+z80d8));	return 4+3+5+4+3;
}
...
int z80RLHL()		// RL (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80RlB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80RLXY()		// RL (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80RlB(*z80Index+z80d8));	return 4+3+5+4+3;
}
...
int z80RRCHL()		// RRC (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80RrcB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80RRCXY()		// RRC (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80RrcB(z80MemReadByte(*z80Index+z80d8)));	return 4+3+5+4+3;
}
...
int z80RRHL()		// RR (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80RrB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80RRXY()		// RR (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80RrB(z80MemReadByte(*z80Index+z80d8)));	return 4+3+5+4+3;
}
...
int z80SLAHL()		// SLA (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80SlaB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80SLAXY()		// SLA (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80SlaB(z80MemReadByte(*z80Index+z80d8)));	return 4+3+5+4+3;
}
...
int z80SRAHL()		// SRA (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80SraB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80SRAXY()		// SRA (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80SraB(z80MemReadByte(*z80Index+z80d8)));	return 4+3+5+4+3;
}
...
int z80SLLHL()		// SLA (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80SllB(z80MemReadByte(z80Regs[Z80HL])));	return 16;
}
...
int z80SRLHL()		// SRL (HL)
{
	z80MemWriteByte(z80Regs[Z80HL],z80SrlB(z80MemReadByte(z80Regs[Z80HL])));	return 4+4+4+3;
}

int z80SRLXY()		// SRL (Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80SrlB(z80MemReadByte(*z80Index+z80d8)));	return 4+3+5+4+3;
}
...
int z80RLD()		// RLD
{
	...
	unsigned char j = z80MemReadByte(z80Regs[Z80HL]);

	*z80RegsB[Z80A] = (i&0xF0)|((j&0xF0)>>4);
	z80MemWriteByte(z80Regs[Z80HL],(j<<4)|(i&0x0F));
...
int z80RRD()		// RRD
{
	...
	*z80RegsB[Z80A] = (i&0xF0)|(j&0x0F);
	z80MemWriteByte(z80Regs[Z80HL],(j>>4)|((i&0x0F)<<4));
	*z80RegsB[Z80F] = (*z80RegsB[Z80F]&0x01) |
		(*z80RegsB[Z80A]&0xA8) |
		((*z80RegsB[Z80A]) ? 0 : 0x40);
	z80parityB(*z80RegsB[Z80A]);
...
{
	*z80RegsB[Z80F] = (*z80RegsB[Z80F] & 0xAD) | 0x10;
	if (!(z80Power[(z80Opcode>>3)&7] & z80MemReadByte(z80Regs[Z80HL])))	*z80RegsB[Z80F] |= 0x40;
	return 4+4+4;
}
...
{
	*z80RegsB[Z80F] = (*z80RegsB[Z80F] & 0xAD) | 0x10;
	if (!(z80Power[(z80Opcode>>3)&7] & z80MemReadByte(*z80Index+z80d8)))	*z80RegsB[Z80F] |= 0x40;
	return 4+3+5+4;
}
...
{
	z80MemWriteByte(z80Regs[Z80HL],z80MemReadByte(z80Regs[Z80HL]) | z80Power[(z80Opcode>>3)&7]);
	return 4+4+4+3;
}

int z80SETXY()		// SET b,(Iz+d8)
{
	z80MemWriteByte(*z80Index+z80d8,z80MemReadByte(*z80Index+z80d8) | z80Power[(z80Opcode>>3)&7]);
	return 4+3+5+4+3;
}
...
int z80EXSPHL()		// EX (SP),HL
{
	...
	z80Regs[Z80HL] = z80MemReadWord(z80Regs[Z80SP]);
	z80MemWriteWord(z80Regs[Z80SP],i);
	return 4+3+4+3+5;
}
...
int z80EXSPXY()		// EX (SP),Iz
{
	...
	*z80Index = z80MemReadWord(z80Regs[Z80SP]);
	z80MemWriteWord(z80Regs[Z80SP],i);
	return 4+3+4+3+5;
}
...
int z80CALL()		// CALL nn
{
	...
	z80Regs[Z80SP]-= 2;	z80MemWriteWord(z80Regs[Z80SP],z80Regs[Z80PC]);		// push PC
...
int z80RET()		// RET
{
	z80Regs[Z80PC] = z80MemReadWord(z80Regs[Z80SP]);	z80Regs[Z80SP]+= 2;	// pop PC
	return 4+3+3;
}
...
int z80OUTn()		// OUT (n),A
{
	...
	z80PortWriteByte(i,*z80RegsB[Z80A]);
	return 8;
}

int z80OUTCr()		// OUT (C),r
{
	z80PortWriteByte(*z80RegsB[Z80C],*z80RegsB[(z80Opcode>>3)&7]);		// write to output port C
	return 4+4+4;
}

int z80OUTD()			// OUTD
{
	z80PortWriteByte(*z80RegsB[Z80C],z80MemReadByte(z80Regs[Z80HL]));		// write to output port C
...
int z80INn()		// IN A,(N)
{
	...
	*z80RegsB[Z80A] = z80PortReadByte(i);	// read from input port N
	return 4+3+4;
}

int z80INrC()		// IN r,(C)
{
	unsigned char i = z80PortReadByte(*z80RegsB[Z80C]);	// read from input port C
...
int z80IND()			// IND
{
	...
	z80MemWriteByte(z80Regs[Z80HL],i);
	z80Regs[Z80HL]--;	*z80RegsB[Z80B]-= 1;
...
inline void z80HandleInterrupt() {
	if (Z80NMIPending)
	{
		...
		z80Regs[Z80SP]-= 2;
		z80MemWriteWord(z80Regs[Z80SP],z80Regs[Z80PC]);
...
void z80Print(FILE *output)
{
	...
	fprintf(output,"\nNext instruction : %02X%02X%02X"
		, z80MemReadByte(z80Regs[Z80PC])
		, z80MemReadByte(z80Regs[Z80PC]+1)
		, z80MemReadByte(z80Regs[Z80PC]+2));
	fprintf(output,"\n\n");
}
