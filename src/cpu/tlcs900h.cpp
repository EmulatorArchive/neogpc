...
//				if (data == 0x55)	DAC_data_w(0,0);
    ...
    // Address is restricted to unsigned char (0x00-0xFF)
    memCPURAM[addr] = data;
    return;
}
...
inline void tlcsMemWriteW(unsigned long addr, unsigned short data)
{
    ...
    {
        tlcsMemWriteByte(addr, data & 0xFF);
        tlcsMemWriteByte(addr+1, data >> 8);
    }
}
...
inline void tlcsMemWriteL(unsigned long addr, unsigned long data)
{
    ...
    {
        tlcsMemWriteByte(addr, (unsigned char)(data & 0xFF));
        tlcsMemWriteByte(addr+1, (unsigned char)((data>>8) & 0xFF));
        tlcsMemWriteByte(addr+2, (unsigned char)((data>>16) & 0xFF));
        tlcsMemWriteByte(addr+3, (unsigned char)((data>>24) & 0xFF));
    }
}
...
inline int VECT_INTLVSET(unsigned char interrupt, unsigned char level)
{
    ...
    {
        case 0x00:
        memCPURAM[0x70] = (memCPURAM[0x70] & 0xf0) |  (level & 0x07);
        break;
        case 0x01:
        memCPURAM[0x71] = (memCPURAM[0x71] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x02:
        memCPURAM[0x73] = (memCPURAM[0x73] & 0xf0) |  (level & 0x07);
        break;
        case 0x03:
        memCPURAM[0x73] = (memCPURAM[0x73] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x04:
        memCPURAM[0x74] = (memCPURAM[0x74] & 0xf0) |  (level & 0x07);
        break;
        case 0x05:
        memCPURAM[0x74] = (memCPURAM[0x74] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x06:
        memCPURAM[0x79] = (memCPURAM[0x79] & 0xf0) |  (level & 0x07);
        break;
        case 0x07:
        memCPURAM[0x79] = (memCPURAM[0x79] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x08:
        memCPURAM[0x7a] = (memCPURAM[0x7a] & 0xf0) |  (level & 0x07);
        break;
        case 0x09:
        memCPURAM[0x7a] = (memCPURAM[0x7a] & 0x0f) | ((level & 0x07)<<4);
        break;
    }
    return 100;
}
...
inline void tlcs_interrupt(int irq)
{
    ...
    int i;
    int level = ((irq&1) ? ((memCPURAM[0x70+(irq>>1)])>>4)&7 : (memCPURAM[0x70+(irq>>1)])&7 );
    if (level==7)
        level = 0;
...
#define DMA0V memCPURAM[0x7C]
#define DMA1V memCPURAM[0x7D]
#define DMA2V memCPURAM[0x7E]
#define DMA3V memCPURAM[0x7F]
...
#define T8RUN memCPURAM[0x20]
#define TREG0 memCPURAM[0x22]
#define TREG1 memCPURAM[0x23]
#define T01MOD memCPURAM[0x24]
#define TREG2 memCPURAM[0x26]
#define TREG3 memCPURAM[0x27]
#define T23MOD memCPURAM[0x28]
int    timer0 = 0;
int    timer1 = 0;
int    timer2 = 0;
int    timer3 = 0;
...
inline void tlcsTimers(int stateChange)
{
    ...
    // // check for updates
    // if (0) {
    //  TREG0 = memCPURAM[0x22] ? memCPURAM[0x22] : 256;
    //  TREG1 = memCPURAM[0x23] ? memCPURAM[0x23] : 256;
    //  TREG2 = memCPURAM[0x26] ? memCPURAM[0x26] : 256;
    //  TREG3 = memCPURAM[0x27] ? memCPURAM[0x27] : 256;
    // }
    // timer 0 + 1
...
// Debugger checks for any breakpoints or running debug instances
	...
	else if ( g_tlcs900hDebugger.state() == DEBUGGER_IDLE )
	{
		...
		for ( int i = 0; i < 100; i++)
		{
			if( breakList[i].active == true && breakList[i].address == gen_regsPC )
			{
...
// Debugger checks for a step command
#ifdef NEOGPC_DEBUGGER
	if ( g_tlcs900hDebugger.state() == DEBUGGER_PAUSE || g_tlcs900hDebugger.state() == DEBUGGER_BREAK )
	{
		// Do nothing while we are paused
		return;
	}
	// Did the user ask for a step? then step
	else if ( g_tlcs900hDebugger.state() == DEBUGGER_STEPIN )
	{
		int elapsed;
	...
	else if ( g_tlcs900hDebugger.state() == DEBUGGER_STEPOVER )
	{
		...
		// See if we are at a call
		if (instr_table[nextOp] == call16 || instr_table[nextOp] == call24 || instr_table[nextOp] == calr)
		{
			skipCode = true;
		}
		...
		else if ( instr_table[nextOp] == decodeB0 )
		{
			...
			if ( decode_tableB0[decodeOp] == callccM300 || decode_tableB0[decodeOp] == callccM301 || decode_tableB0[decodeOp] == callccM302 || 
				...
				decode_tableB0[decodeOp] == callccM309 || decode_tableB0[decodeOp] == callccM30A || decode_tableB0[decodeOp] == callccM30B || 
				decode_tableB0[decodeOp] == callccM30C || decode_tableB0[decodeOp] == callccM30D || decode_tableB0[decodeOp] == callccM30E || 
				decode_tableB0[decodeOp] == callccM30F )
			{
				skipCode = true;
			}
		}
		...
		else if ( instr_table[nextOp] == decodeF0 )
		{
			...
			if ( decode_tableF0[decodeOp] == callccM300 || decode_tableF0[decodeOp] == callccM301 || decode_tableF0[decodeOp] == callccM302 || 
				...
				decode_tableF0[decodeOp] == callccM309 || decode_tableF0[decodeOp] == callccM30A || decode_tableF0[decodeOp] == callccM30B || 
				decode_tableF0[decodeOp] == callccM30C || decode_tableF0[decodeOp] == callccM30D || decode_tableF0[decodeOp] == callccM30E || 
				decode_tableF0[decodeOp] == callccM30F )
			{
				skipCode = true;
			}
		}
		...
		while(cycles > 0 && g_tlcs900hDebugger.state() == DEBUGGER_IDLE)
		{
			...
			do
			{
				...
				if ( g_tlcs900hDebugger.state() == DEBUGGER_PAUSE || 
					g_tlcs900hDebugger.state() == DEBUGGER_BREAK)
				{
					break;
				}
			} 
